...
\STATE{matriz covarianza $=$  matrizDeCovarianza(etiquetados)}
...
\STATE{vector vectorInicial$=$  vector(cant_filas(covarianza))}
...
\STATE{matriz covarianza}
\STATE{vector nuevo}
\FOR{i=1 \TO size(medias)}
	\FOR{j=1 \TO cant\_filas(entrada)}
		\STATE{nuevoVector_j = entrada_{(j,i)}- medias_i }
	\ENDFOR
